...
def render_v0(gvc_params, viewpoint_camera, pc : GaussianModel, pipe, bg_color : torch.Tensor, scaling_modifier = 1.0, 
           override_color = None, stage="fine", cam_type=None, visible_mask=None, retain_grad=False):
    """
    Render the scene. 
    ...
    ############### test mode 1 ###############
    ###########################################
    if gvc_params["GVC_testmode"] == 0 or gvc_params["GVC_testmode"] == 1:
        is_training = pc.get_color_mlp.training
        ...
        # mean3D 부터 먼저 할당
        # means3D = pc.get_xyz # origianl 4DGS
        means3D = xyz

        # 원인 파악됨
        # generate neural gaussian에서 xyz를 많이 생성해서 xyz 사이즈가 커짐
        ...
        ''' 
        # Spherical Harmonics 계산부 --> 필요 없음. 일단 주석처리
        # print(opacity.max())
    ...
    if gvc_params["GVC_testmode"] == 2:
        ...
        if "coarse" in stage:
            ...
            if pipe.compute_cov3D_python: # False임
                ...
                scales = scaling # Scaffold-GS
                rotations = rot # Scaffold-GS
