...
def render_ablation(model_path, name, iteration, views, gaussians, pipeline, background, cam_type, gvc_params):
    render_path = os.path.join(model_path, name, "ours_{}".format(iteration), "renders")
    gts_path = os.path.join(model_path, name, "ours_{}".format(iteration), "gt")
...
pipeline : PipelineParams, skip_train : bool, skip_test : bool, skip_video: bool, canonical_frame_render : bool, gvc_params):
    with torch.no_grad():
        ...
        render_ablation(model_path, name, iteration, scene, gaussians, pipeline, background, cam_type, gvc_params)
