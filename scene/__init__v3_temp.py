...
class Scene:
    ...
    def __init__(self, args : ModelParams, gaussians : GaussianModel, load_iteration=None, 
                 shuffle=True, resolution_scales=[1.0], load_coarse=False, static_dynamic=None):
        """b
    ...
    def save(self, iteration, stage):
        if self.static_dynamic is None:
            if stage == "coarse":
        ...
        elif self.static_dynamic == "static": 
            if stage == "coarse":
                point_cloud_path = os.path.join(self.model_path, "point_cloud/coarse_static_iteration_{}".format(iteration))
            else:
                point_cloud_path = os.path.join(self.model_path, "point_cloud/static_iteration_{}".format(iteration))
            self.gaussians.save_ply(os.path.join(point_cloud_path, "point_cloud.ply"))
            # self.gaussians.save_deformation(point_cloud_path) # 저장 안함
            # scaffold-gs
            self.gaussians.save_mlp_checkpoints(point_cloud_path)
        elif self.static_dynamic == "dynamic":
            if stage == "coarse":
                point_cloud_path = os.path.join(self.model_path, "point_cloud/coarse_dynamic_iteration_{}".format(iteration))
            else:
                point_cloud_path = os.path.join(self.model_path, "point_cloud/dynamic_iteration_{}".format(iteration))
            self.gaussians.save_ply(os.path.join(point_cloud_path, "point_cloud.ply"))
            self.gaussians.save_deformation(point_cloud_path)
            # scaffold-gs
            self.gaussians.save_mlp_checkpoints(point_cloud_path)
