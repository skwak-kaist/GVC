...
import copy
@torch.no_grad()
def render_training_image(scene, gaussians, viewpoints, render_func, pipe, background, stage, iteration, time_now, dataset_type, gvc_params):
    def render(gvc_params, gaussians, viewpoint, path, scaling, cam_type):
        # scaling_copy = gaussians._scaling
        render_pkg = render_func(gvc_params, viewpoint, gaussians, pipe, background, stage=stage, cam_type=cam_type)
    ...
    for idx in range(len(viewpoints)):
        ...
        render(gvc_params, gaussians,viewpoints[idx],image_save_path,scaling = 1,cam_type=dataset_type)
